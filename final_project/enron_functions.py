...
def get_k_best(data_dict, features_list, k):
    ...
    print "{0} most influential: {1}\n".format(k, k_best_features.keys())
    return k_best_features

def add_poi_interaction_fraction(data_dict, features_list):
    ...
    for record in data_dict:
        ...
        if is_valid:
            ...
            person['poi_interaction_fraction'] = float(poi_messages) / total_messages
            
        else:
            person['poi_interaction_fraction'] = 'NaN'
    features_list += ['poi_interaction_fraction']

def add_financial_sum(data_dict, features_list):
    ...
    for record in data_dict:
        ...
        for field in fields:
            if person[field] == 'NaN':
                is_valid = False
        if is_valid:
            person['financial_sum'] = float(sum([person[field] for field in fields]))
...
if __name__ == '__main__':
    ...
    #add_poi_interaction_fraction(data_dict, features_list)
    #visualize(data_dict, 'total_stock_value', 'poi_interaction_fraction')

    #add_financial_sum(data_dict, features_list)
    #visualize(data_dict, 'poi_interaction_fraction', 'financial_sum')
